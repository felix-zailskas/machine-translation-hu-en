...
for path in tqdm(Path("cfg").glob("*.yaml")):
    ...
    word_input_sentences, word_output_sentences, word_max_tokens = load_dataset(
        in_lang=in_lang, out_lang=out_lang, dataset=dataset
    )
    ...
    (
        ...
        (
            word_input_word2idx,
            word_output_word2idx,
            word_input_idx2word,
            word_output_idx2word,
        ),
    ) = load_models_from_config(config, word_max_tokens)
    word_encoder.eval()
    word_decoder.eval()
    (
        char_encoder,
    ...
    word_one_gram_scores[path] = word_one_gram_bleu_scores.mean()
    word_two_gram_scores[path] = word_two_gram_bleu_scores.mean()
...
for key, value in sorted(char_ranks.items(), key=lambda item: item[1]):
    print(f"{key}: {value[0]}:: bleu:{value[1][0]} nist:{value[1][1]}")
